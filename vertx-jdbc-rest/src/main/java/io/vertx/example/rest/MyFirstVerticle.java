...
public class MyFirstVerticle extends AbstractVerticle {
	...
	public void start(Future<Void> fut) {
		...
		jdbc = JDBCClient.createShared(vertx, config(), "WhiskyDatasource");
		Handler<AsyncResult<HttpServer>> next2 = http -> completeStartup(http, fut);
		Handler<AsyncResult<Void>> next1 = nothing -> startWebApp(next2);

		startBackend(connection -> createSomeData(connection, next1, fut), fut);
	}
	...
	private void startBackend(Handler<AsyncResult<SQLConnection>> next, Future<Void> fut) {
		...
		jdbc.getConnection(ar -> {
			if (ar.failed()) {
				System.out.println(">>>>>>>>>>>>>>>>>> startBackend : ar.failed");
				fut.fail(ar.cause());
			} else {
				System.out.println(">>>>>>>>>>>>>>>>>> startBackend : ar.success");
				next.handle(ar);
			}
		});
	}
	...
	private void createSomeData(AsyncResult<SQLConnection> result, Handler<AsyncResult<Void>> next, Future<Void> fut) {
		...
		if (result.failed()) {
			System.out.println(">>>>>>>>>>>>>>>>>>>> createSomeData result: failed");
			fut.fail(result.cause());
		} else {
			System.out.println(">>>>>>>>>>>>>>>>>>>> createSomeData result: else");
			SQLConnection connection = result.result();
			connection.execute(
					"CREATE TABLE IF NOT EXISTS Whisky (id INTEGER, name varchar(100), origin varchar" + "(100))",
					ar -> {
						if (ar.failed()) {
							fut.fail(ar.cause());
							connection.close();
						...
						connection.query("SELECT * FROM Whisky", select -> {
							...
							if (select.result().getNumRows() == 0) {
								System.out.println(">>>>>>>>>>>>>>>>>>>> createSomeData : Creating data");
	...
	private void completeStartup(AsyncResult<HttpServer> http, Future<Void> fut) {
		System.out.println(">>>>>>>>>>>>>>>>>>>  completeStartup");
		if (http.succeeded()) {
			fut.complete();
		} else {
			fut.fail(http.cause());
	...
	private void updateOne(RoutingContext routingContext) {
		...
		if (id == null || json == null) {
			...
			jdbc.getConnection(ar -> update(id, json, ar.result(), (whisky) -> {
				if (whisky.failed()) {
					routingContext.response().setStatusCode(404).end();
				} else {
		...
		connection.queryWithParams("SELECT * FROM Whisky WHERE id=?", new JsonArray().add(id), ar -> {
			if (ar.failed()) {
				...
				if (ar.result().getNumRows() >= 1) {
					System.out.println(">>>>>> result: " + ar.result().getRows().get(0));
