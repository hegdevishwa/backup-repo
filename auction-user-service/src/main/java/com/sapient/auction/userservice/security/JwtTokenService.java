...
public class JwtTokenService {
	 ...
	 *             when the Authentication header couldn't be verified or didn't
	 *             match the expected username.
	 */
	...
	private String verifyJwt(String token) {
		try {
			String username = Jwts.parser().setSigningKey(secret).parseClaimsJws(token).getBody().get("user",
					String.class);
			return username;
	...
	private String buildJwtToken(String username) {
		String token = Jwts.builder().signWith(SignatureAlgorithm.HS512, secret)
				.setPayload(new JSONObject().put("user", username).toString()).compact();
		return token;
	}
